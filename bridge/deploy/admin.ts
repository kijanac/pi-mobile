...
} from "../../packages/protocol/src/version.ts";
import { renderBridgeCloudInit } from "../../packages/protocol/src/cloud-init.ts";
...
function renderCloudInit(args: string[]): void {
  const [tsAuthKey, bridgeHostname] = args;
  if (!tsAuthKey || !bridgeHostname) {
    throw new Error("usage: admin.ts render-cloud-init <ts-auth-key> <bridge-hostname>");
  }
  process.stdout.write(renderBridgeCloudInit({ tsAuthKey, bridgeHostname }));
}
...
function updateRelease(args: string[]): void {
  ...
  emitLines([
    "update",
    version,
    assetUrl(release, "bridge-release.json"),
    assetUrl(release, "bridge-release.json.sig"),
  ]);
}
...
function updateManifest(args: string[]): void {
  ...
  if (!releasePath || !manifestPath || !expectedVersion) {
    throw new Error("usage: admin.ts update-manifest <release.json> <bridge-release.json> <expected-version>");
  }

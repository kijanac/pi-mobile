...
step "directories"
install -d -o "$USER_NAME" -g "$USER_NAME" -m 0755 "$INSTALL_DIR" "$DATA_DIR"
...
if [[ ! -f "$ETC_DIR/env" ]]; then
  ...
  echo "  → EDIT IT NOW to set ANTHROPIC_API_KEY"
else
...
Next steps:
  1. Edit /etc/pi-bridge/env to set ANTHROPIC_API_KEY
  2. From your laptop, run deploy/deploy.sh to push the source

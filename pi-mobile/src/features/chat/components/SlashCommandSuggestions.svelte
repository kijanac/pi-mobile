...
<div class="hairline-b max-h-56 overflow-y-auto px-2 py-2">
  {#if loading}
    <div class="px-2 py-2 text-[12px] text-[color:var(--color-fg-faint)]">loading commands…</div>
  {:else if error}
    <div class="px-2 py-2 text-[12px] text-[color:var(--color-danger)]">{error}</div>
  {:else if entries.length === 0}
    <div class="px-2 py-2 text-[12px] text-[color:var(--color-fg-faint)]">no matches</div>
  {:else}
    <div class="space-y-1">
      {#each entries as entry, index (entry.kind + entry.name)}
        ...
        >
          ...
          <span class="min-w-0 flex-1">
            <span class="block truncate text-[12px] text-[color:var(--color-fg)]">{label(entry)}</span>
            <span class="block truncate text-[11px] text-[color:var(--color-fg-muted)]">{entry.description}</span>
          </span>
        </button>
      {/each}
    </div>

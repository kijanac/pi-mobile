...
{#if !onboarding.loaded}
  <main class="text-copy flex flex-1 items-center justify-center p-4 text-[color:var(--color-fg-muted)]">
    loading onboarding…
  </main>
{:else}
  ...
  <main class="flex min-h-0 flex-1 flex-col">
    <header class="flex items-center justify-between gap-3 border-b border-[color:var(--color-border)] px-3 py-[calc(env(safe-area-inset-top)+12px)] pb-3">
      ...
      <h1 class="text-title font-medium">bridge onboarding</h1>
        ...
        <CarouselContent class="h-full -ml-3">
          <CarouselItem class="min-h-0 overflow-y-auto pl-3">
            <OnboardingPanel eyebrow="step 1" title="before you start">
              <p class="text-copy text-[color:var(--color-fg-muted)]">
          ...
          <CarouselItem class="min-h-0 overflow-y-auto pl-3">
            <OnboardingPanel eyebrow="step 3" title="copy cloud-init">
              ...
              <div>
                ...
                <Textarea id="cloud_init" readonly value={onboarding.cloudInit} rows={15} class="text-code min-h-0 resize-none bg-[color:var(--color-bg)] font-mono" />
              </div>
          ...
          <CarouselItem class="min-h-0 overflow-y-auto pl-3">
            <OnboardingPanel eyebrow="step 4" title="wait for bridge">
              ...
              <div class="text-copy rounded-[var(--radius-md)] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-3 text-[color:var(--color-fg-muted)]">
          ...
          <CarouselItem class="min-h-0 overflow-y-auto pl-3">
            <OnboardingPanel eyebrow="step 5" title="provider sign-in">
              ...
              {#if onboarding.authError}
                <p class="text-meta rounded-[var(--radius-md)] border border-[color:var(--color-border)] bg-[color:var(--color-bg)] p-2 text-[color:var(--color-danger)]">{onboarding.authError}</p>
              {/if}
          ...
          <CarouselItem class="min-h-0 overflow-y-auto pl-3">
            <OnboardingPanel eyebrow="done" title="bridge ready">
              <div class="rounded-[var(--radius-lg)] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-4 text-center">
                ...
                <p class="text-title font-medium">pi-mobile is connected to your bridge.</p>
                <p class="text-copy mt-1 text-[color:var(--color-fg-muted)]">Provider sign-in is available from a session’s action menu when you’re ready.</p>
              </div>
